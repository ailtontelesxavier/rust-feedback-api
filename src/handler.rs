...
pub async fn get_feedback_handler(
    ...
    let query_result = sqlx::query_as!(
        Feedback,
        "SELECT * FROM feedbacks WHERE id = $1", id)
        .fetch_one(&data.db)
        .await;
...
pub async fn feedback_list_handler(
    ...
    let limit = opts.limit.unwrap_or(10).min(100) as i64;
    let offset = ((opts.page.unwrap_or(1).saturating_sub(1)) * limit as usize) as i64;
